...
fn test_amplitude_to_db() {
    let S = arr2(&[[1.0], [0.5]]);
    let db = amplitude_to_db(&S, None, None, None).unwrap();
...
fn test_db_to_amplitude() {
    ...
    let amp = db_to_amplitude(&S_db, None).unwrap();
...
fn test_power_to_db() {
    let S = arr2(&[[1.0], [0.25]]);
    let db = power_to_db(&S, None, None, None).unwrap();
    assert!(db[[0, 0]] > db[[1, 0]]);
}
...
fn test_db_to_power() {
    let S_db = arr2(&[[0.0], [-6.0]]);
    let power = db_to_power(&S_db, None).unwrap();
